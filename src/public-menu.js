...
const allowedFields = item => ({ name:String(item.name||''),price:Number(item.price)||0,type:item.type==='promo'?'promo':'food',category:String(item.category||'Otros'),ingredients:item.ingredients?String(item.ingredients):null });
...
export function renderPublicMenu(items,storeName='PJ Delivery') {
  ...
  const content=[...groups.entries()].sort(([a],[b])=>a.localeCompare(b,'es')).map(([category,rows])=>`<section><h2>${escapeHtml(category)}</h2>${rows.sort((a,b)=>a.name.localeCompare(b.name,'es')).map(item=>`<article><div><h3>${escapeHtml(item.name)}</h3>${item.ingredients?`<p>${escapeHtml(item.ingredients)}</p>`:''}</div><strong>${new Intl.NumberFormat('es-AR',{style:'currency',currency:'ARS',maximumFractionDigits:0}).format(item.price)}</strong></article>`).join('')}</section>`).join('');
