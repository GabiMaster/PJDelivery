...
test('calcula el efectivo físico final descontando ambos egresos del checkpoint',()=>{const value=cashSummary(orders,[{type:'permanente',amount:1000},{type:'vuelto_cliente',amount:700}]);assert.equal(value.grossCash,17000);assert.equal(value.pickupCash,5000);assert.equal(value.courierNetCash,8200);assert.equal(value.cashBeforeReplenishment,13200);assert.equal(value.physicalCashAfterReplenishment,11500);assert.equal(value.cashBeforeReplenishment-value.permanentExpenses-value.customerChange,value.physicalCashAfterReplenishment);assert.equal('netCash' in value,false);assert.equal(value.replenishmentNeeded,1700)});
